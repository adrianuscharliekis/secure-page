...
export const getCountries = async (url) => {
  try {
    ...
    if (
      ...
      const newData=data.data.records.sort((a, b) => a.name.localeCompare(b.name));
      return newData;
    }
    return [];
  } catch (error) {
...
export const createBooking = async (formData, countries = []) => {
  ...
  const param = {
    // ...additionalPayload, // This was in the original snippet, ensure it's defined in the scope if needed.
    trxToko: "",
    ...
    emailPemesan: contactDetails.email,
    namaPemesan: contactDetails.fullName,
  };
